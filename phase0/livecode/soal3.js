...
function ganjilGenapAsianGames(date, data) {
    ...
    if(date>=1 && date<=31){
        ...
        for(let i = 0 ; i<data.length; i++){
            const dataBaru=parseInt(data[i].plat.split(' ')[1]);
            const PlatGenap = dataBaru % 2 ===0;
            
            if(data[i].type === 'Mobil' && date % 2 ===0 && PlatGenap){
                nilai++;
            }else if(data[i].type === 'Mobil' && date % 2 !==0 && !PlatGenap){
                nilai++;
            }
        }
        return nilai;
...
//     let nilai = 0;
//     for(let i = 0 ; i<data.length; i++){
//         const dataMobil = data[i];
//         if(dataMobil.type == 'Mobil' && (date>=1 && date<=31)){
//                 nilai +=1;
//         }else if (){
//             return `invalid dates`;
//         }
