...
function countProfit(shoppers) {
    ...
    for(let j = 0; j<listBarang.length;j++){
      ...
      let product = listBarang[j][0];
     
      let money = listBarang[j][1];
      let stock = listBarang[j][2];
      let namalist =[]
      let hasilProfit =0;
    for(let i = 0; i<shoppers.length;i++){
        if(shoppers[i].product===product && shoppers[i].amount<=stock){
         
                namalist.push(shoppers[i].name);
                stock -=shoppers[i].amount;
                hasilProfit += (shoppers[i].amount*money);
        }
    }
